...
const getStockClass = (qty) => { if (qty <= 0) return "out"; if (qty <= 10) return "low"; return "ok"; };
const getStockLabel = (qty) => { if (qty <= 0) return "Hết hàng"; if (qty <= 10) return "Sắp hết"; return qty; };
...
const SanPhamManager = () => {
  ...
  const initialForm = {
    maSP: "", tenSP: "", loaiSanPhamId: "", donVi: "Cái",
    giaBan: "", soLuongTon: 0, trangThai: true, hinhAnhs: [],
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        loaiSanPhamId: formData.loaiSanPhamId ? Number(formData.loaiSanPhamId) : null,
  ...
  const handleSetMainImage = async (imageId) => {
    try {
      ...
      fetchProducts();
    } catch { alert("Lỗi khi đặt hình chính!"); }
  };
  ...
  const totalLow = products.filter((p) => p.soLuongTon > 0 && p.soLuongTon <= 10).length;
          ...
          <div className="spm-root">
            <div className="spm-page-header">
              <div>
                ...
                <div className="spm-breadcrumb">
                  Sản phẩm / {isEdit ? `Sửa: ${formData.tenSP || "..."}` : "Thêm mới"}
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleSave}>
              <div className="spm-form-layout">
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <div className="spm-form-card">
                    <div className="spm-form-card-header">📋 Thông tin cơ bản</div>
                    <div className="spm-form-body">
                      <div className="spm-field-grid">
                        <div className="spm-field">
                          <label className="spm-label">Mã sản phẩm <span className="req">*</span></label>
                          <input
                              className="spm-input" placeholder="VD: SP001" required
                              value={formData.maSP}
                        ...
                        <div className="spm-field">
                          ...
                          >
                            {["Cái","Hộp","Thùng","Lọ","Túi","Kg","Lít","Bộ","Gói","Cuộn","License","Gói dịch vụ"].map(
                                (u) => <option key={u}>{u}</option>
                            )}
                          </select>
                        </div>
                        <div className="spm-field full">
                          <label className="spm-label">Tên sản phẩm <span className="req">*</span></label>
                          <input
                              className="spm-input" placeholder="Nhập tên sản phẩm..." required
                        ...
                        <div className="spm-field full">
                          ...
                          <div className="spm-select-with-action">
                            <select
                                ...
                                onChange={(e) => field("loaiSanPhamId", e.target.value)}
                            >
                    ...
                    <div className="spm-form-body">
                      <div className="spm-field-grid">
                        <div className="spm-field">
                          ...
                          <input
                              className="spm-input" type="number" min="0" step="1000" placeholder="0"
                              value={formData.giaBan}
                              onChange={(e) => field("giaBan", e.target.value)}
                          />
                          {formData.giaBan > 0 && (
                              <span className="spm-hint">≈ {fmtVND(formData.giaBan)}</span>
                          )}
                        </div>
                        <div className="spm-field">
                          <label className="spm-label">Số lượng tồn kho</label>
                          <input
                              className="spm-input" type="number" min="0"
                              value={formData.soLuongTon}
                              onChange={(e) => field("soLuongTon", parseInt(e.target.value) || 0)}
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="spm-form-card">
                    ...
                    <div className="spm-form-body">
                      <div className="spm-toggle-wrap">
                        ...
                        {formData.trangThai ? "🟢 Đang kinh doanh" : "⚪ Ngừng kinh doanh"}
                      </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {isEdit && curProd && (
                      <div style={{
                        ...
                        justifyContent: "space-between", fontSize: 11.5, color: "#9ca3af"
                      }}>
                ...
                <div>
                  <div className="spm-form-card">
                    <div className="spm-form-card-header">🖼️ Hình ảnh sản phẩm</div>
                    <div className="spm-form-body">
                      {isEdit ? (
                          <>
                            {formData.hinhAnhs?.length > 0 ? (
                                <div className="spm-img-grid">
                                  {formData.hinhAnhs.map((img) => (
                                      <div key={img.id} className={`spm-img-item ${img.isMain ? "main-img" : ""}`}>
                                        <img src={`http://localhost:8081/crm-ver1${img.urlHinhAnh}`} alt="" />
                                        {img.isMain && <span className="spm-img-badge-main">CHÍNH</span>}
                                        <div className="spm-img-overlay">
                                          <button type="button" className="spm-img-action-btn" disabled={img.isMain}
                                                  onClick={() => handleSetMainImage(img.id)}>⭐</button>
                                          <button type="button" className="spm-img-action-btn"
                                                  onClick={() => handleDeleteImage(img.id)}>🗑️</button>
                                        </div>
                                      </div>
                                  ))}
                                </div>
                            ...
                            <label className="spm-upload-zone">
                              ...
                              <div className="spm-upload-text">
                                <strong>Nhấn để tải ảnh lên</strong><br />PNG, JPG tối đa 5MB
                              </div>
                            </label>
                          </>
                      ) : (
                          <div style={{ textAlign: "center", padding: "20px 0", color: "#9ca3af", fontSize: 12.5 }}>
              ...
              <div style={{
                ...
                border: "1px solid #e5e7eb", padding: "14px 22px",
                display: "flex", justifyContent: "flex-end", gap: 10
              }}>
                <button type="button" className="spm-btn spm-btn-outline"
                        onClick={() => { setView("list"); setEditId(null); }}>
                  Hủy
                </button>
  ...
  return (
      <>
        ...
        <div className="spm-root">
          <div className="spm-page-header">
            <div>
              <div className="spm-page-title">Quản lý <span>Sản phẩm</span></div>
              <div className="spm-breadcrumb">Tổng cộng {products.length} sản phẩm trong hệ thống</div>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button
                  ...
                  onClick={() => { setFormData(initialForm); setEditId(null); setView("add"); }}
          ...
          <div className="spm-stats-row">
            <div className="spm-stat-card">
              ...
              <div><div className="spm-stat-val">{products.length}</div><div className="spm-stat-lbl">Tổng sản phẩm</div></div>
            </div>
            <div className="spm-stat-card">
              <div className="spm-stat-icon green">✅</div>
              <div><div className="spm-stat-val">{totalActive}</div><div className="spm-stat-lbl">Đang kinh doanh</div></div>
            </div>
            <div className="spm-stat-card">
              <div className="spm-stat-icon amber">⚠️</div>
              <div><div className="spm-stat-val">{totalLow}</div><div className="spm-stat-lbl">Sắp hết hàng</div></div>
            </div>
            <div className="spm-stat-card">
              <div className="spm-stat-icon red">🚫</div>
              <div><div className="spm-stat-val">{totalOut}</div><div className="spm-stat-lbl">Hết hàng</div></div>
            </div>
          </div>
          ...
          <div className="spm-card">
            <div className="spm-card-header">
              <div className="spm-search-wrap">
                ...
                <input
                    className="spm-search" placeholder="Tìm theo mã, tên sản phẩm..."
            ...
            <div style={{ overflowX: "auto" }}>
              <table className="spm-table">
                <thead>
                <tr>
                  <th style={{ width: 60 }} className="center">Ảnh</th>
                  <th>Sản phẩm</th>
                  <th>Loại SP</th>
                  <th>Đơn vị</th>
                  <th>Giá bán</th>
                ...
                {filtered.length === 0 ? (
                    ...
                    filtered.map((p) => {
                      ...
                      const cat = categories.find((c) => c.id === p.loaiSanPhamId || c.id === Number(p.loaiSanPhamId));
                      return (
                          <tr key={p.id}>
                            <td className="center">
                              {displayUrl ? (
                                  <img src={displayUrl} alt="" className="spm-product-thumb" />
                              ) : (
                                  <div style={{
                                    width: 46, height: 46, borderRadius: 8, background: "#f3f4f6",
                            ...
                            <td className="center">
                          <span className={`spm-stock-badge ${getStockClass(p.soLuongTon)}`}>
                            {getStockLabel(p.soLuongTon)}
                          </span>
                            </td>
                            <td className="center">
                          <span className={`spm-status-dot ${p.trangThai ? "active" : "inactive"}`}>
                            {p.trangThai ? "Kinh doanh" : "Ngừng"}
                          </span>
                            </td>
                            <td style={{ fontSize: 12, color: "#6b7280" }}>{fmtDate(p.updatedAt)}</td>
                            <td className="center">
                              <div className="spm-action-group" style={{ justifyContent: "center" }}>
                                <button
                                    className="spm-btn spm-btn-outline spm-btn-sm"
                                    onClick={() => { setFormData(p); setEditId(p.id); setView("edit"); }}
                                >✏️ Sửa</button>
                              </div>
                            </td>
                          </tr>
